...
const tr = (path, fallback = path) => {
  ...
  for (const part of parts) {
    if (!ref || typeof ref !== 'object') return FALLBACK_STRINGS[lang]?.[path] || fallback;
    ref = ref[part];
  }
...
function showCooldownPopup(waitSec) {
  ...
  if (!message) {
    const defaultMsgs = { en: `You can resend in ${waitSec}s.`, pt: `Podes reenviar em ${waitSec}s.` };
    message = defaultMsgs[getLanguage()] || defaultMsgs.en;
  }
...
function showLoginError(message) { if (errText) errText.textContent = message || defaultErrText; hide(okMsg); show(errMsg); }
function resetLoginError() { if (errText) errText.textContent = defaultErrText; hide(errMsg); }
function showOtpError(message) { if (otpErrText) otpErrText.textContent = message || defaultOtpErrText; hide(otpOk); show(otpErr); }
function resetOtpError() { if (otpErrText) otpErrText.textContent = defaultOtpErrText; hide(otpErr); }
...
async function requestCaptcha(timeoutMs = 120000) {
  ...
  if (isOtpVisible) {
    ...
    if (otpActions) otpStep.insertBefore(captchaGroup, otpActions);
    else otpStep.appendChild(captchaGroup);
...
function markSessionActive() {
  const storageValue = { loggedIn: true, loggedInAt: Date.now() };
...
form.addEventListener('submit', async (e) => {
  ...
  if (!email || password.length < 8) { showLoginError(defaultErrText); return; }
  ...
  try {
    ...
    await callAuthWorker('login_password', { email, password, captchaToken: tokenA });
    const tokenB = await requestCaptcha();
    await callAuthWorker('otp_request', { email, captchaToken: tokenB });
    ...
    if (isTooManyRequestsError(err)) {
      ...
      } else showLoginError(tr('login.networkRetry'));
    }
  } finally {
    isSubmitting = false;
    resetButton(submitBtn);
...
btnVerify.addEventListener('click', async () => {
  ...
  try {
    ...
    if (isTooManyRequestsError(err)) {
      ...
      } else showOtpError(tr('login.networkRetry'));
    }
  } finally {
    isVerifying = false;
    resetCaptcha();
...
btnResend.addEventListener('click', async () => {
  ...
  try {
    ...
    await callAuthWorker('otp_request', { email: loginEmail, captchaToken: tokenD });
    nextOtpRequestAt = Date.now() + 60_000;
    const okText = okMsg?.querySelector('span');
    ...
    if (isTooManyRequestsError(err)) {
      nextOtpRequestAt = Date.now() + 60_000;
      showLoginError(tr('login.tooManyWait60'));
    } else showLoginError(tr('login.captchaRetry'));
  } finally {
